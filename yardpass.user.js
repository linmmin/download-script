...
// @updateURL    https://axzile.corp.amazon.com/-/carthamus/script/amazon-yms-yard-pass-generator2
// @downloadURL  https://axzile.corp.amazon.com/-/carthamus/script/amazon-yms-yard-pass-generator2
...
(function() {
    ...
    DOCK_AND_WAIT: 'dock_and_wait', // NEW: For docking scenarios
...
.driver-action-btn.dock-wait:hover {
    ...
    const options = {
        ...
        showDockAndWait: false, // NEW option for docking scenarios
...
// Track if truck has its own VRID (BEFORE any copying from trailer)
    ...
    function generateSwapBodyInstructionHtml(t, driverAction) {
        ...
        const dropBodyWord = dropCount === 1 ? t.swapBody : t.swapBodies;
        const pickupBodyWord = pickupCount === 1 ? t.swapBody : t.swapBodies;

        if (dropCount > 0 && pickupCount > 0) {
            const dropText = t.dropXEmpty.replace('{x}', dropCount).replace('{body}', dropBodyWord);
            const pickupText = t.pickUpX.replace('{x}', pickupCount).replace('{body}', pickupBodyWord);
            instructionText = `${dropText}<br>↓<br>${pickupText}`;
            instructionClass = 'swap-instruction exchange';
        } else if (dropCount > 0 && pickupCount === 0) {
            instructionText = t.dropXEmpty.replace('{x}', dropCount).replace('{body}', dropBodyWord);
            instructionClass = 'swap-instruction drop-only';
        } else if (dropCount === 0 && pickupCount > 0) {
            instructionText = t.pickUpX.replace('{x}', pickupCount).replace('{body}', pickupBodyWord);
        ...
        if (instructionText) {
            return `
            <div class="info-box">
                <div class="info-label">📦 ${t.swapBodyOperation}:</div>
                <div class="${instructionClass}">${instructionText}</div>
            </div>
            `;
        }
    ...
    function generateYardPass(data, lang, driverAction = null) {
...
const isParkAndWaitAction = driverAction && driverAction.action === DRIVER_ACTIONS.PARK_AND_WAIT;
        ...
        if (isSwapBodyExchange) {
            attachableSectionsHtml += generateSwapBodyInstructionHtml(t, driverAction);
        }
        ...
        if (trailers.length > 0) {
            if (( isDropAndSoloExitAction || !showCombinedVrid) && (trailerVrids.length > 0 || trailerIsas.length > 0)) {
                ...
                <div class="info-box">
                    <div class="info-label">${t.vridIsaTrailer}</div>
                    <div class="key-info">
            ...
            if (trailerIds.length > 0) {
                attachableSectionsHtml += `
                <div class="info-box">
                    <div class="info-label">${trailerIds.length > 1 ? t.trailerIds : t.trailerId}</div>
            ...
            if (!allSameLocation ||  isDropAndSoloExitAction) {
                ...
                if (uniqueTrailerSpots.length === 1) {
                    ...
                    if (isDockLocation(trailerSpot)) {
                        const dockType = trailerSpot.startsWith('OB') ? t.outbound : t.inbound;
                        const dockNumber = trailerSpot.substring(2);
                        attachableSectionsHtml += `
                        <div class="info-box">
                            <div class="location-instruction">${t.dockAt} ${dockType} ${t.door} ${dockNumber}</div>
                            <div class="location-value">${trailerSpot}</div>
                            ${ isDropAndSoloExitAction ? `<div class="exit-instruction">${t.dropAndExit}</div>` : ''}
                        </div>
                        `;
                    } else {
                        attachableSectionsHtml += `
                        <div class="info-box">
                            <div class="location-instruction">${t.dropTrailerAt}</div>
                            <div class="location-value">${trailerSpot}</div>
                            ${ isDropAndSoloExitAction ? `<div class="exit-instruction">${t.dropAndExit}</div>` : ''}
                        </div>
                        `;
                    ...
                    trailers.forEach((trailer) => {
                        if (trailer.spot !== data.truck.spot ||  isDropAndSoloExitAction) {
                            ...
                            let instruction;
                            if (isDock) {
                                const dockType = trailer.spot.startsWith('OB') ? t.outbound : t.inbound;
                                const dockNumber = trailer.spot.substring(2);
                                instruction = `${t.dockAt} ${dockType} ${t.door} ${dockNumber}`;
                            } else {
                                instruction = t.dropTrailerAt;
                            }
                            attachableSectionsHtml += `
                            <div class="info-box">
                                <div class="info-label">${trailer.id}:</div>
                                <div class="location-instruction">${instruction}</div>
                    ...
                    if ( isDropAndSoloExitAction) {
                        attachableSectionsHtml += `
                        <div class="info-box">
                            <div class="exit-instruction">${t.dropAndExit}</div>
                        </div>
                        `;
                    }
                }
        ...
        if (swapBodies.length > 0) {
            if ((isDropAndSoloExitAction || !showCombinedVrid) && (swapBodyVrids.length > 0 || swapBodyIsas.length > 0)) {
                attachableSectionsHtml += `
                <div class="info-box">
                    <div class="info-label">${t.vridIsaSwapBody}</div>
                    <div class="key-info">
            ...
            if (swapBodyIds.length > 0) {
                attachableSectionsHtml += `
                <div class="info-box">
                    <div class="info-label">${swapBodyIds.length > 1 ? t.swapBodyIds : t.swapBodyId}</div>
           ...
           if (!allSameLocation || isDropAndSoloExitAction) {
                ...
                if (uniqueSwapBodySpots.length === 1) {
                    ...
                    if (isDockLocation(swapBodySpot)) {
                        const dockType = swapBodySpot.startsWith('OB') ? t.outbound : t.inbound;
                        const dockNumber = swapBodySpot.substring(2);
                        attachableSectionsHtml += `
                        <div class="info-box">
                            <div class="location-instruction">${t.dockAt} ${dockType} ${t.door} ${dockNumber}</div>
                            <div class="location-value">${swapBodySpot}</div>
                            ${isDropAndSoloExitAction ? `<div class="exit-instruction">${t.dropSwapBodyAndExit}</div>` : ''}
                        </div>
                        `;
                    } else {
                        attachableSectionsHtml += `
                        <div class="info-box">
                            <div class="location-instruction">${t.dropSwapBodyAt}</div>
                            <div class="location-value">${swapBodySpot}</div>
                            ${ isDropAndSoloExitAction ? `<div class="exit-instruction">${t.dropSwapBodyAndExit}</div>` : ''}
                        </div>
                        `;
                    ...
                    swapBodies.forEach((swapBody) => {
                        if (swapBody.spot !== data.truck.spot ||  isDropAndSoloExitAction) {
                            ...
                            let instruction;
                            if (isDock) {
                                const dockType = swapBody.spot.startsWith('OB') ? t.outbound : t.inbound;
                                const dockNumber = swapBody.spot.substring(2);
                                instruction = `${t.dockAt} ${dockType} ${t.door} ${dockNumber}`;
                            } else {
                                instruction = t.dropSwapBodyAt;
                            }
                            attachableSectionsHtml += `
                            <div class="info-box">
                    ...
                    if ( isDropAndSoloExitAction) {
                        attachableSectionsHtml += `
                        <div class="info-box">
                            <div class="exit-instruction">${t.dropSwapBodyAndExit}</div>
                        </div>
                        `;
                    }
                }
        ...
        if (!isDropAndSoloExitAction) {
            ...
            if (isSwapBodyExchange) {
                ...
                if (truckHasVrid) {
                    vridSectionHtml = `
                    <div class="info-box">
                        <div class="info-label">${t.vridIsaTruck}</div>
                ...
                vridSectionHtml = `
                <div class="info-box">
                    <div class="info-label">${t.vridIsa}</div>
                    <div class="key-info">${data.truck.vrid || data.truck.loadId || '---'}</div>
                </div>
                `;
        ...
        if (driverAction) {
            if (driverAction.action === DRIVER_ACTIONS.PARK_AND_WAIT) {
                ...
                const pickupExitText = hasOnlySwapBodies ? t.pickUpSwapBodyAndExit : t.pickUpAndExit;
                driverActionInstruction = `<div class="exit-instruction">${pickupExitText}</div>`;
            } else if (driverAction.action === DRIVER_ACTIONS.DROP_AND_SOLO_EXIT) {
                // This is handled in the attachables section with exit instruction
                driverActionInstruction = `<div class="exit-instruction">${t.soloExit}</div>`;
                ...
                <div class="custom-instruction-box">
                    <div class="info-label">${t.customInstruction}</div>
        ...
        if ( !isDropAndSoloExitAction) {
            let truckInstruction;
            // If pickup and exit is selected, use "Connect to trailer/swap body at"
            if (isPickupAndExit) {
                const connectText = hasOnlySwapBodies ? t.connectToSwapBodyAt : t.connectToTrailerAt;
                truckInstruction = connectText;
            } else if (isCombiTourTruckAtDock) {
                const connectText = hasOnlySwapBodies ? t.connectToSwapBodyAt : t.connectToTrailerAt;
                truckInstruction = connectText;
            } else if (isSwapBodyDropOnly) {
                truckInstruction = t.dropSwapBodyAt;
            } else if (isDockLocation(data.truck.spot)) {
                const dockType = data.truck.spot.startsWith('OB') ? t.outbound : t.inbound;
                const dockNumber = data.truck.spot.substring(2);
                truckInstruction = `${t.dockAt} ${dockType} ${t.door} ${dockNumber}`;
            } else {
                truckInstruction = `${t.parkAt}`;
            }
    ...
    (isSwapBodyExchange && swapBodyParkAndWait)
            ...
            if (isCombiTourTruckAtDock) {
                const pickupExitText = hasOnlySwapBodies ? t.pickUpSwapBodyAndExit : t.pickUpAndExit;
                finalDriverActionInstruction = `<div class="exit-instruction">${pickupExitText}</div>`;
            } else if (isSwapBodyDropOnly) {
                finalDriverActionInstruction = `<div class="exit-instruction">${t.dropSwapBodyAndExit}</div>`;
                ...
                const pickupExitText = t.pickUpSwapBodyAndExit;
                finalDriverActionInstruction = `<div class="exit-instruction">${pickupExitText}</div>`;
            }

            truckSectionHtml = `
            <div class="info-box">
                <div class="info-label">${t.truckId}</div>
                <div class="key-info">${data.truck.plate}</div>
            </div>

            <div class="info-box">
                <div class="location-instruction">${truckInstruction}</div>
                <div class="location-value">${data.truck.spot}</div>
                ${showWaitInLounge ? `<div class="wait-instruction">${t.andWaitInLounge}</div>` : ''}
                ${finalDriverActionInstruction}
            </div>
            `;
        } else {
            ...
            if (isDropAndSoloExitAction) {
                truckSectionHtml += `
                <div class="info-box">
                    <div class="exit-instruction">${t.soloExit}</div>
                </div>
                `;
            }
        }

        // Determine pick up label
        let pickUpLabel = t.pickUpTrailer;
        if (hasOnlySwapBodies) {
            pickUpLabel = t.pickUpSwapBody;
        }
        ...
        // UPDATED: For swap body exchange with park and wait, show pager AND pickup boxes
        ...
        // UPDATED: Show pickup box for:
        // - Normal trailer scenarios where driver parks and waits
        // - Swap body exchange scenarios where driver parks and waits (swapBodyParkAndWait)
...
<head>
    <meta charset="UTF-8">
    <title>${t.yardPass} - ${data.siteCode}</title>
    <style>
        @page {
...
<body>
    <div class="container">
        <div class="header">
            ${t.yardPass} - ${data.siteCode}
        </div>

        <div class="tips-section">
            <div class="tips-welcome">🚛 ${t.welcomeTo} ${data.siteCode}</div>
            <ul class="tips-list">
                <li>
                    <span class="tip-emoji">🦺</span>
                    <span class="tip-text">${t.tip1}</span>
                </li>
                <li>
                    <span class="tip-emoji">🚛</span>
                    <span class="tip-text">${t.tip2}</span>
                </li>
                <li>
                    <span class="tip-emoji">⚠️</span>
                    <span class="tip-text">${t.tip3}</span>
                </li>
                <li>
                    <span class="tip-emoji">🔑</span>
                    <span class="tip-text">${t.tip4}</span>
                </li>
            </ul>
        </div>
        ...
        ${showPagerBox ? `
        <div class="pager-box">
            <div class="pager-header">${t.pagerNo}</div>
        ...
        <div class="print-info">
            ${t.printed} ${data.printTime} | ${t.printedBy} ${data.printedBy}
        </div>
    </div>
</body>
</html>`;
